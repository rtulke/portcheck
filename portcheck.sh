...
# IP and Port from hex in a readable format
parse_addr() {
...
# Go through all lines of /proc/net/tcp (except headers)
tail -n +2 /proc/net/tcp | while read -r line; do
    ...
    for pid in $(ls -d /proc/[0-9]* 2>/dev/null); do
        ...
        # Search all FDs after the inode
        for fd in "$pid"/fd/*; do
            [ -L "$fd" ] || continue
            link=$(readlink "$fd" 2>/dev/null)
            if [[ "$link" == socket:* ]]; then
                fd_inode=$(awk -F'\\[|\\]' '{print $2}' <<< "$link")
